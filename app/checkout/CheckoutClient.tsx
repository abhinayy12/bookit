...
export default function CheckoutClient() {
  ...
  const { exp, qty, promo, setPromo } = useBooking();
  ...
  const apply = () => {
    if (code.toUpperCase() === "FLAT100") setPromo("FLAT100", 100);
      ...
      setPromo("SAVE10", Math.round(exp.price * qty * 0.1));
    else setPromo("", 0);
  };
  ...
  const pay = () => {
    if (!name || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || !agree)
  ...
  return (
    <section className="py-6 grid md:grid-cols-[1fr_320px] gap-6">
      <div>
        ...
        <div className="bg-[#F3F4F6] rounded-xl p-4 space-y-3">
          <div className="grid md:grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="mt-1 w-full h-10 rounded-md bg-white px-3 border"
          ...
          <div className="flex items-end gap-3">
            ...
            <button
              onClick={apply}
              className="h-10 px-4 rounded-md bg-[#111827] text-white"
            >
              Apply
            </button>
          </div>
      ...
      <div className="pt-8 md:pt-0">
        ...
        <button
          onClick={pay}
          className="w-full h-10 mt-4 rounded-md bg-[#FACC15] text-[#111827] font-medium"
        >
